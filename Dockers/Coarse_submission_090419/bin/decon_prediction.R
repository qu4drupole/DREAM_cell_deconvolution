...
# read meta data
input_df <- suppressMessages(readr::read_csv("input/input.csv"))
print(head(input_df))
...
do_decon <- function(expression_path, dataset_name, data_scale){
  ...
  pred_matrix <- sapply(1:length(cell_models), function(x) {print(names(cell_models[x]))
                                                            make_prediction(cell_models[[x]], ex)})
  ...
  pred_df <- melt(pred_df, id.vars = c('dataset.name', 'sample.id'), variable.name = 'cell.type', 
                  value.name = 'measured') %>%
    arrange(sample.id)
  pred_df$cell.type <- rep(names(cell_models), nrow(ex)) #had to add this bc I changed sapply
  return(pred_df)
}
